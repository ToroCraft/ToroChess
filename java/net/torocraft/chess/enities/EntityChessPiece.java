...
public abstract class EntityChessPiece extends EntityCreature implements IChessPiece {
	...
	private boolean isRunTick() {
		return world.getTotalWorldTime() % 80L == 0L;
	}
	...
	private void findGame() {
		...
		if(pos != null){
			...
			game = ((BlockChessControl)world.getBlockState(pos).getBlock()).getGame();
	...
	private boolean blockIsChessControl(BlockPos pos) {
		...
		return world.getBlockState(pos).getBlock() instanceof BlockChessControl;
	}
	...
	public void writeEntityToNBT(NBTTagCompound tagCompound) {
		...
		if(isSet(chessPosition)){
			tagCompound.setString(NBT_POSITION_KEY, chessPosition);
		}else{
			tagCompound.removeTag(NBT_POSITION_KEY);
		}
